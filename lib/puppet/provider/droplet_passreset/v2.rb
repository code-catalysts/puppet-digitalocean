...
Puppet::Type.type(:droplet_passreset).provide(:v2) do
  ...
  def exists?
    ...
    response = @client.droplet.password_reset(name)
    Puppet.info("password reset response #{response} exists")
    #@property_hash[:ensure] == :present
  end

  def create # rubocop:disable Metrics/AbcSize
    Puppet.info("yolo biatch #{name}")
  end
